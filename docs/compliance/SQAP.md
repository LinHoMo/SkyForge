...
> **注意**: 当前团队规模下，QA 独立性难以完全满足。采用"工具自动化 + 人工审查"双重机制弥补：自动化检查由 CI（独立于开发者的机器环境）执行；人工审查由非代码作者团队成员执行。
...
| 文档完整性审计 | 每次 Release | 人工 + 脚本 | 8 份计划文档完整 | QA团队 | 审计报告 |
...
| 结构覆盖率 | 每次生成运行 | `coverage_analyzer.py` + `gcov_collector.py` | 真实 gcov/lcov 达标；静态分析只做缺口定位 | 验证团队 | 覆盖率报告 |
| 耦合分析 | 每次生成运行 | `coupling_analyzer.py` | 数据耦合/控制耦合通过 | 验证团队 | 耦合分析报告 |
...
| 文档完整率 | 100% 8 份计划文档存在 | 脚本检查 | QA团队 | 每次Release |
...
| 结构覆盖率 (DAL-C) | 100% 语句 | gcov/lcov 真实收集 | 验证团队 | 每次生成 |
| 结构覆盖率 (DAL-A) | 100% MC/DC | GCC 14.2+ lcov 2.0+ | 验证团队 | 每次生成 |
| DO-178C 目标满足率 | >= 80% (Level C) | do178_objectives.py | QA团队 | 每次生成 |
| 编译成功率 | 100% | 真实 GCC；模拟执行不计入 A-5.3 | 开发团队 | 每次生成 |
