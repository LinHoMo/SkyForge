...
from skyforge_engine.digital_twin.hil_adapter_base import HILMode
...
/* === 预定义测试环境变量 === */
/* 为AI生成的代码提供标准化的测试接口 */
static double raw_input = 0.0;          /* 当前输入值 */
static double filtered_output = 0.0;    /* 当前输出值 */
static bool fault_detected = false;     /* 故障检测标志 */
static double sample_rate = 100.0;      /* 采样率 */
static double cutoff_frequency = 10.0;  /* 截止频率 */
static int step_count = 0;              /* 步数计数 */
...
class VirtualMCU:
    ...
    def __init__(
        ...
        # tests; task execution always passes an explicit immutable snapshot.
        self.use_real_gcc = use_real_gcc
        # 缓存 GCC 是否可用（首次调用时检测）
    ...
    def _compiler_version(self) -> str | None:
        try:
            result = subprocess.run(
                [self.gcc_path, "--version"], capture_output=True, text=True,
    ...
    def compile(
        ...
        if "double filter" not in code and "filter(" not in code:
            ...
            if log_callback:
                log_callback("TERMINAL", "info", "代码未定义 filter 函数，将在 test_harness 中自动添加 wrapper")
        ...
        effective_use_real_gcc = (
            settings.USE_REAL_GCC
            if self.use_real_gcc is None
            else self.use_real_gcc
        )
        if not effective_use_real_gcc:
    ...
    def _generate_test_harness(self, user_code: str, assert_code: str = "") -> str:
        ...
        # 检测用户代码中的函数名
        # 查找 double xxx(double 或 double xxx(double, ...) 模式
...
"""
    ...
    def _create_adapter(self):
        """根据 HILConfig 创建对应适配器。"""
        if self.hil_config.mode == HILMode.VIRTUAL:
            return None
        elif self.hil_config.mode == HILMode.SERIAL:
            return SerialHILAdapter(self.hil_config)
        elif self.hil_config.mode == HILMode.QEMU:
            return QEMUAdapter(self.hil_config)
        else:
            raise ValueError(f"不支持的 HIL 模式: {self.hil_config.mode}")
        ...
        if self._adapter is not None:
            if asyncio.iscoroutinefunction(self._adapter.run):
                ...
                result = await loop.run_in_executor(
                    None, self._adapter.run, input_waveform
                )
            return {
                "output_waveform": getattr(result, "output_waveform", None),
                "method": getattr(result, "method", ""),
                "status": getattr(result, "status", "error"),
            }

        # 降级到现有 VIRTUAL/mock 实现
        input_data = np.array(input_waveform, dtype=np.float64)
        run_result = self.run("", input_data, used_mock=True)
        return {
            "output_waveform": (
                run_result.output_data.tolist()
                if len(run_result.output_data) > 0
                else []
            ),
            "method": "virtual_mock",
