...
class TestPipelineHILIntegration(unittest.TestCase):
    ...
    def tearDown(self) -> None:
        _reset_singletons()
        # 恢复 HIL 默认禁用，避免测试方法中途切换后污染后续测试文件
        os.environ["HIL_ENABLED"] = "false"
        reset_hil_manager()
    ...
    def test_pipeline_hil_reject_aborts(self) -> None:
        """HIL 启用且需求评审被拒绝时，流水线应正确中止（丢弃并行生成的契约）。

        验证设计文档 14.7.4 §4 优化：HIL 需求评审与契约生成并行执行，
        当 HIL 拒绝时契约结果被丢弃，流水线中止。
        """
        # 切换到 HIL 启用并重置单例使设置生效
        os.environ["HIL_ENABLED"] = "true"
        reset_hil_manager()
        manager = get_hil_manager()

        async def scenario() -> dict:
            async def run_pipe() -> dict:
                return await run_pipeline("实现一个低通滤波器，截止频率10Hz")

            async def reject_after_pending() -> dict:
                # 等待 requirement_review 出现在 pending 列表
                for _ in range(200):
                    pending = manager.get_pending_approvals()
                    if pending:
                        break
                    ...
                    self.fail("等待 requirement_review pending 出现超时")
                request_id = pending[0]["request_id"]
                self.assertEqual(pending[0]["checkpoint"], "requirement_review")
                return await manager.reject(
                    request_id=request_id,
                    comments="需求不清晰",
                    reviewer="alice",
            ...
            # Mock pipeline 的 get_hil_manager 以返回同一个 manager 实例
            with patch("skyforge_engine.pipeline.get_hil_manager", return_value=manager):
                pipe_task = asyncio.create_task(run_pipe())
                reject_task = asyncio.create_task(reject_after_pending())
