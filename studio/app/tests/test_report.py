...
from skyforge_engine.report import (
    TraceEntry,
    build_matrix,
    check_objectives,
    generate_report,
...
class TestDO178Objectives(unittest.TestCase):
    ...
    def test_do178_objectives_status_values(self) -> None:
        """每项目标 status ∈ {满足, 部分满足, 未满足, 不适用}。"""
        for obj in self.objectives:
            self.assertIn(obj.status, ("满足", "部分满足", "未满足", "不适用"))
...
class TestReportAPIRoutes(unittest.TestCase):
    ...
    def test_report_route_returns_html_and_matrix_and_objectives(self) -> None:
        """POST /api/report 返回 report_html
        / traceability_matrix / do178_objectives。"""
        import asyncio
