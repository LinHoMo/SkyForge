...
export const MOCK_VIOLATIONS: MisraViolation[] = [
	...
	},
	{
		...
		line: 30,
		message:
			"Both operands of an expression shall have the same essential type category.",
	},
	{
		rule: "MISRA-Rule-9.1",
		category: "Mandatory",
	...
	},
	{
		...
		line: 40,
		message:
			"Pointer parameter 'f' in filter_get_alpha should be declared as pointer-to-const (auto-repaired).",
	},
];
...
export const MISRA_RULE_DOCS: Record<string, string> = {
	"MISRA-Rule-8.1":
		"Required: All declarations shall be qualified with appropriate type specifiers.",
	"MISRA-Rule-9.1":
		"Mandatory: The value of an object with automatic storage shall not be read before being set.",
	"MISRA-Rule-10.4":
		"Required: Both operands of an operator shall have the same essential type category.",
	"MISRA-Rule-8.13":
		"Advisory: A pointer parameter in a function prototype should be declared as pointer-to-const if the pointer is not used to modify the addressed object.",
};
...
export const MOCK_REPAIR_HISTORY: RepairIteration[] = [
	{
		...
		description:
			"Cppcheck 扫描发现 4 个违规，LLM 自动修复 3 处：变量初始化、类型转换、显式类型限定。Rule-8.13 需第 2 轮处理。",
	},
	{
		round: 2,
		...
		description:
			"修复 filter_get_alpha 的指针参数为 const，符合 Rule-8.13 Advisory。全部违规清零。",
	},
];
			...
			items: [
				{
					...
					passed: true,
					assert_code:
						'assert(sample_rate > 0 && "[CON-001-PRE-000] 违反前置条件: 采样率必须大于 0");',
				},
				{
					id: "CON-001-PRE-001",
					expression: "raw_value <= 65535",
					...
					passed: true,
					assert_code:
						'assert(raw_value <= 65535 && "[CON-001-PRE-001] 违反前置条件: 原始值超范围");',
				},
			],
		},
		{
			...
			items: [
				{
					...
					passed: true,
					assert_code:
						'assert(filtered_value >= 0 && filtered_value <= 65535 && "[CON-001-POST-000] 违反后置条件: 输出越界");',
				},
				{
					id: "CON-001-POST-001",
					expression:
					...
					passed: false,
					failure_reason:
						"浮点精度边界：fc=10Hz, fs=100Hz 时 alpha=0.0909，浮点误差 1e-7 触发严格 assert 失败。实际可接受，建议增加容差 eps=1e-6。",
					assert_code:
						'assert(fabs(filtered_value - (alpha * raw_value + (1 - alpha) * prev)) < 1e-6 && "[CON-001-POST-001] 违反后置条件: IIR 公式不匹配");',
				},
			],
		},
		{
			...
			items: [
				{
					...
					passed: true,
					assert_code:
						'assert(alpha >= 0.0f && alpha <= 1.0f && "[CON-001-INV-000] 违反不变式: alpha 超范围");',
				},
			],
		},
		{
			...
			items: [
				{
					...
					passed: true,
					assert_code:
						'assert((sample_rate == 0 ? filtered_value == prev_filtered : 1) && "[CON-001-FLT-000] 违反故障处理: 采样率异常未保持输出");',
				},
			],
		},
	],
...
export const MOCK_SIM_LOGS: AgentLog[] = [
	{
		agent: "SYSTEM",
		level: "info",
		thought: "$ gcc -fsanitize=address code.c test_harness.c -o sim",
	},
	{
		agent: "SYSTEM",
		level: "success",
		thought: "编译成功，test_harness.c 已注入 6 条契约断言（Patch 2）",
	},
	{
		agent: "SYSTEM",
		level: "info",
		thought: "$ ./sim --steps 200 --input sine_10Hz.bin",
	},
	{
		agent: "TERMINAL",
		level: "info",
		thought: "[sim] step 0: input=32768, output=2982 (初始化瞬态)",
	},
	{
		agent: "TERMINAL",
		level: "info",
		thought: "[sim] step 50: input=52768, output=51023 (稳态跟踪)",
	},
	{
		agent: "TERMINAL",
		level: "info",
		thought: "[sim] step 100: input=12768, output=14539 (稳态跟踪)",
	},
	{
		agent: "TERMINAL",
		level: "success",
		thought: "[sim] 全部 200 步仿真完成，6/6 契约断言通过",
	},
	{
		agent: "SYSTEM",
		level: "success",
		thought: "✅ 数字孪生仿真通过，契约零违约",
	},
];
